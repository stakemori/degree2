...
@cached_function
def _rankin_cohen_bracket_func(Q, rnames, unames):
    '''
    Let
    ...
    def rankin_cohen(flist):
        ...
        for (i, _), pol in u_dict.iteritems():
            ...
            for longtpl, v in pol.dict().iteritems():
                ...
                psum += v*mul([f._differential_operator_monomial(*t)
...
def rankin_cohen_pair_det2_sym(j, f, g):
    ...
    # det(R+S)
    detRpS = -QQ(1)/QQ(4)*r12**2 + r11*r22 + r22*s11 - \
        QQ(1)/QQ(2)*r12*s12 - QQ(1)/QQ(4)*s12**2 + r11*s22 + s11*s22

    Q2 = (2*k - 1) * (2*l - 1) * detRpS - (2*k - 1) * (2*k + 2*l - 1)\
        * detS - (2*l - 1)*(2*k + 2*l - 1)*detR
    Q = QQ(4)**(-1) * Q2 * Q +\
